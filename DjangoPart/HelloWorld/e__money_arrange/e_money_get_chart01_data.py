...
def e_money_get_chart01_data(request):
    ...
    if time_scale == 'given_span':
        ...
        for sd_idx in range(sd_duration + 1):   # 循环查询范围内日期的 time_record
            ...
            # 查询该天的 money_record
            cur.execute(" select ID, AMOUNT FROM e_money_record WHERE DATE_STR = '%s' AND INOUT = '%s'; "
                        % (date_str, 'out'))
        ...
        # 查询全部的 money_record
        cur.execute("select ID, AMOUNT, DATE_STR FROM e_money_record WHERE INOUT = '%s'; " % 'out')
        for item in cur.fetchall():
