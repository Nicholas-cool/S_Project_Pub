...
def add_money_record(record_name, record_date, record_inout, record_type, record_position, record_amount,
    ...
    # 添加记录信息
    cur.execute(""" 
        INSERT INTO e_money_record (NAME, TYPE, AMOUNT, INOUT, POSITION, DESCRIPTION, DATE_STR, STATUS) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?) 
...
def add_money_transfer_record(record_name, record_date, record_from_position, record_to_position,
    ...
    # 添加记录信息
    cur.execute(""" 
        INSERT INTO e_money_record (NAME, TYPE, AMOUNT, INOUT, POSITION, DESCRIPTION, DATE_STR, FEE, STATUS) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) 
...
def e_upload_money_records(request):
    ...
    cur.execute(""" select NAME, NAME_EN from e_money_position; """)
    if 'unknown' not in [_[1] for _ in cur.fetchall()]:
        cur.execute(""" insert into e_money_position (NAME, NAME_EN, TYPE, MONEY, HIDDEN) 
