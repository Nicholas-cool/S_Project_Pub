...
def e_money_get_chart_history01_data(request):
    ...
    if check_position != 'all':
        # 查询得到当前的资金余量
        cur.execute(" select MONEY, NAME from e_money_position where NAME_EN = '%s'; " % check_position)
        ...
        for sd_idx in range(sd_duration-1, -1, -1):   # 循环查询范围内日期的 money_record（逆序）
            ...
            # 查询该天的 money_record
            cur.execute(""" select ID, AMOUNT, INOUT, POSITION, FEE FROM e_money_record WHERE DATE_STR = '%s'; 
            """ % date_str_next)
        ...
        # 查询得到当前的资金余量
        cur.execute(" select MONEY, NAME from e_money_position; ")
