...
def e_money_get_chart03_data(request):
    ...
    if time_scale == 'given_span':
        ...
        for sd_idx in range(sd_duration + 1):   # 循环查询范围内日期的 time_record
            ...
            # 查询该天的 money_record
            cur.execute(" select ID, TYPE, AMOUNT FROM e_money_record WHERE DATE_STR = '%s' AND INOUT = '%s'; "
                        % (date_str, inout_type))
        ...
        # 查询全部的 money_record
        cur.execute("select ID, TYPE, AMOUNT FROM e_money_record WHERE INOUT = '%s'; " % inout_type)
        for item in cur.fetchall():
            d_type, d_amount = item[1], float(item[2])
